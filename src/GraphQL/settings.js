import { GraphQLClient } from "graphql-request";

//export const serverClient = new GraphQLClient("http://localhost:8080/graphql");
//export const serverClient = new GraphQLClient("https://morning-spire-11480.herokuapp.com/graphql/");

// Last online
export const serverClient = new GraphQLClient("https://deploy-orchestre-api-test.herokuapp.com/graphql/");

//export const serverClient = new GraphQLClient("http://localhost:5003/graphql");